...
import TopicCardSlide from "./TopicCardSlideTw";
...
/* allow code blocks / inner pagers to scroll horizontally */
...
export default function SubjectPager({ subjectItem }) {
  ...
  const touchMetaRef = useRef({ scrollerX: null, startLeft: 0 });
  ...
  // ✅ touch swipe (updated: don't navigate if inner horizontal pager can scroll)
  useEffect(() => {
    const el = pagerRef.current;
    if (!el) return;
    ...
    const onTouchStart = (e) => {
      ...
      touchTargetRef.current = startTarget;

      const scrollerX = findScrollableX(startTarget, el);
      touchMetaRef.current = {
        scrollerX,
        startLeft: scrollerX ? scrollerX.scrollLeft : 0,
      };
    };
    ...
    const onTouchEnd = (e) => {
      ...
      if (absX > 60 && absX > absY * 1.2) {
        const { scrollerX, startLeft } = touchMetaRef.current || {};
        const scrollIntent = -dx; // swipe left => want scroll right (positive)

        // if inner scroller exists and either it can scroll in that direction OR it already scrolled during the gesture, do NOT navigate cards
        if (
          scrollerX &&
          scrollerX.scrollWidth > scrollerX.clientWidth + 4 &&
          (canScrollX(scrollerX, scrollIntent) || scrollerX.scrollLeft !== startLeft)
        ) {
          return;
        }
      ...
      // vertical gesture (keeps your existing behavior)
  ...
  return (
    <div
      ...
      className={[
        ...
        // ✅ allow native horizontal scroll inside inner pagers (markdown pages)
