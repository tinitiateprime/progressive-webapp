...
import SubjectPager from "./SubjectPager3";
...
export default function MobileCatalog({ catalog }) {
  ...
  // ✅ SAFE defaults first (avoid Next.js hydration/localStorage issues)
  ...
  useEffect(() => {
    try {
      const savedTheme = localStorage.getItem("mcat_theme");
      const savedFont = localStorage.getItem("mcat_font");

      if (savedTheme === "dark" || savedTheme === "light") setTheme(savedTheme);
      if (savedFont && FONT_OPTIONS.some((f) => f.id === savedFont)) setFontId(savedFont);
    } catch {
