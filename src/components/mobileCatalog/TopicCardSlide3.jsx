...
import MarkdownViewer from "../../markdown/MarkdownViewer3";
...
export default function TopicCardSlide({
  ...
  const setZoom = (fnOrValue) => {
    if (typeof onZoomChange !== "function") return;
    if (typeof fnOrValue === "function") {
      onZoomChange((z) => clamp(fnOrValue(z), ZOOM_MIN, ZOOM_MAX));
    } else {
      onZoomChange(clamp(Number(fnOrValue) || 1, ZOOM_MIN, ZOOM_MAX));
    }
  };

  const zoomIn = () => setZoom((z) => z + ZOOM_STEP);
  const zoomOut = () => setZoom((z) => z - ZOOM_STEP);
  ...
  // ✅ Ctrl/Cmd + wheel zoom ONLY content; do NOT let pager move the card
  useEffect(() => {
  ...
  return (
    ...
    >
      ...
      >
              ...
              {isActive ? (
                <div
                  className={[
                    ...
                    "dark:border-white/14 dark:bg-slate-900/90",
                  ].join(" ")}
                  aria-label="Zoom controls"
                  title="Ctrl/Cmd + wheel to zoom • Ctrl/Cmd+0 = reset"
                >
                  <button
                    type="button"
                    onClick={zoomOut}
                    className="h-[26px] w-[30px] grid place-items-center text-slate-900/80 dark:text-slate-200/85"
                    aria-label="Zoom out"
                  >
                    <FiMinus size={14} />
                  </button>
                  ...
                  <button
                    ...
                    onClick={zoomReset}
                    className={[
                      "h-[26px] w-[30px] grid place-items-center",
                      "border-x border-slate-900/10 dark:border-white/12",
                      "text-slate-900/75 dark:text-slate-200/75",
                    ].join(" ")}
        ...
        {/* BODY (✅ no vertical scroll; consistent width; no clipping) */}
        <div
          ref={viewportRef}
          className={[
            "p-[14px] md:p-4",
        ...
        >
          {!isActive ? (
            ...
            // ✅ IMPORTANT: w-full + min-w-0 prevents wide code blocks from forcing the whole markdown wider
            <div
